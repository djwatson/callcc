...
;; (define (hash-table-ref ht key) (hash-table-ref/default ht key #f))
...
;; 	(cond
;; 	 ((not cur)
;; 	  (vector-set! table idx (cons key default))
;; 	  (hash-table-size-set! ht (+ 1 (hash-table-size ht))))
;; 	 ((cmp (car cur) key) (set-cdr! cur (updater (cdr cur))))
;; 	 (else (loop (hash-modulo (+ idx 1) len))))))))
...
;; 	 (else (loop (hash-modulo (+ idx 1) len))))))))
;; (define (hash-table-walk ht visitor)
;;   (let* ((entries (hash-table-entries ht))
;; 	(len (vector-length entries)))
;;     (do ((i 0 (+ i 1)))
;; 	((= i len))
;;       (sys:FOREIGN_CALL "SCM_DISPLAY" i 0)
;;       (sys:FOREIGN_CALL "SCM_DISPLAY" #\newline 0)
;;       (let ((entry (vector-ref entries i)))
;; 	(when entry
;; 	  (visitor (car entry) (cdr entry)))))))
